...
export class TranscriptionService {
    private languages: Array<[string, Array<[string, string?]>]> = [
        ['Afrikaans', ['af-ZA']],
    ...
    private speechTranscription: typeof SpeechRecognition | undefined = window.SpeechRecognition || (window as any).webkitSpeechRecognition;
    public isTranscriptionSupported: boolean = false;
    private transcriptionRunning: boolean = false;
    private transcription: SpeechRecognition | undefined;
    private transcripts: Array<any> = [];
    private isBgTransparent: boolean = false;
    private isPinned: boolean = false;
    private isHidden: boolean = true;
    private isPersistentMode: boolean = false;
    private isPersistent: boolean = false;
    private showOnMessage: boolean = true;

    constructor() {
        this.isTranscriptionSupported = this.isSupported();
    ...
    private isSupported(): boolean {
        return !!this.speechTranscription;
    }
    ...
    private init(): void {
        ...
        if (this.speechTranscription) {
            ...
            this.transcription.lang = 'en-US'; // Default language

            this.transcription.onstart = () => {
                console.log('Transcription started');
            };

            this.transcription.onresult = (e: SpeechRecognitionEvent) => {
                const current = e.resultIndex;
                const transcript = e.results[current][0].transcript;
                const transcriptionData: TranscriptionData = {
                    type: 'transcript',
                    room_id: 'room_id', // Replace with actual room_id
                    peer_name: 'peer_name', // Replace with actual peer_name
            ...
            this.transcription.onerror = (event: SpeechRecognitionErrorEvent) => {
                console.error('Transcription error', event.error);
            };

            this.transcription.onend = () => {
                console.log('Transcription stopped');
                if (this.isPersistentMode && this.isPersistent && this.transcriptionRunning) {
                    setTimeout(() => {
                        this.start();
                    }, 2000);
                } else {
            ...
            console.warn(
                'This browser does not support Transcription. Check supported browsers: https://developer.mozilla.org/en-US/docs/Web/API/Web_Speech_API#browser_compatibility'
            );
        }
    }

    private sendTranscript(transcriptionData: TranscriptionData): void {
        // Implement the logic to send the transcription data
    ...
    private handleTranscript(transcriptionData: TranscriptionData): void {
       
        // Implement the logic to handle the received transcription data
  ...
  save() {
    ...
    private handleLanguages(): void {
        // Implement the logic to handle languages
